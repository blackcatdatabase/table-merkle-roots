...
namespace BlackCat\Database\Packages\MerkleRoots\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\MerkleRoots\Dto\MerkleRootDto;
use BlackCat\Database\Packages\MerkleRoots\Mapper\MerkleRootDtoMapper;
 ...
 */
final class MerkleRootsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private MerkleRootRepository $merkleRootRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'merkle_roots' => $this->merkleRootRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\MerkleRoots\ModuleException("Record {$idStr} not found for locking.");
            }
