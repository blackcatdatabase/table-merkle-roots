...
final class Criteria extends BaseCriteria
{
    ...
    {
        return [ 'id', 'subject_table', 'period_start', 'period_end', 'root_hash', 'proof_uri', 'status', 'leaf_count', 'created_at' ];
    }

    /** Columns used for full-text LIKE/ILIKE searches. */
    protected function searchable(): array
    {
        return [ 'subject_table', 'proof_uri', 'status' ];
    }

/** Columns allowed in ORDER BY (falls back to filterable() when empty). */
protected function sortable(): array
{
    return [ 'id', 'subject_table', 'period_start', 'period_end', 'proof_uri', 'status', 'leaf_count', 'created_at' ];
}
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\MerkleRoots\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\MerkleRoots\Definitions::softDeleteColumn();
