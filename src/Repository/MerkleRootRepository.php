...
use BlackCat\Database\Packages\MerkleRoots\Repository\MerkleRootRepositoryInterface;
...
class MerkleRootRepository implements MerkleRootRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'subject_table', 'period_start', 'period_end' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\MerkleRoots\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'root_hash', 'leaf_count' ]);
          return count($rows);
        }
    ...
    public function getBySubjectTableAndPeriodStartAndPeriodEnd(string $subjectTable, \DateTimeInterface|string $periodStart, \DateTimeInterface|string $periodEnd, bool $asDto = false): array|\BlackCat\Database\Packages\MerkleRoots\Dto\MerkleRootDto|null {
        return $this->getByUnique([ 'subject_table' => $subjectTable, 'period_start' => $periodStart, 'period_end' => $periodEnd ], $asDto);
    }
    ...
    public function getIdBySubjectTableAndPeriodStartAndPeriodEnd(string $subjectTable, \DateTimeInterface|string $periodStart, \DateTimeInterface|string $periodEnd) {
        $row = $this->getBySubjectTableAndPeriodStartAndPeriodEnd($subjectTable, $periodStart, $periodEnd, false);
