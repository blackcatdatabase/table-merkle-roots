...
namespace BlackCat\Database\Packages\MerkleRoots\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\MerkleRoots\Definitions;
use BlackCat\Database\Packages\MerkleRoots\Criteria;
use BlackCat\Database\Packages\MerkleRoots\Dto\MerkleRootDto as Dto;
use BlackCat\Database\Packages\MerkleRoots\Mapper\MerkleRootDtoMapper as RowMapper;
...
final class MerkleRootRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\MerkleRoots\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'subject_table', 'period_start', 'period_end' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\MerkleRoots\Definitions::table());
            return $bulk->upsertMany($rows, [ 'subject_table', 'period_start', 'period_end' ], [ 'root_hash', 'leaf_count' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\MerkleRoots\Dto\MerkleRootDto|null */
    public function getBySubjectTableAndPeriodStartAndPeriodEnd(string $subjectTable, \DateTimeInterface|string $periodStart, \DateTimeInterface|string $periodEnd, bool $asDto = false): array|\BlackCat\Database\Packages\MerkleRoots\Dto\MerkleRootDto|null {
        $row = $this->getByUnique([ 'subject_table' => $subjectTable, 'period_start' => $periodStart, 'period_end' => $periodEnd ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\MerkleRoots\Mapper\MerkleRootDtoMapper::fromRow($row);
    }
    public function existsBySubjectTableAndPeriodStartAndPeriodEnd(string $subjectTable, \DateTimeInterface|string $periodStart, \DateTimeInterface|string $periodEnd): bool {
        $where = 't.' . Ident::q($this->db, 'subject_table') . ' = :uniq_subject_table' . ' AND ' . 't.' . Ident::q($this->db, 'period_start') . ' = :uniq_period_start' . ' AND ' . 't.' . Ident::q($this->db, 'period_end') . ' = :uniq_period_end';
        return $this->exists($where, [ 'uniq_subject_table' => $subjectTable, 'uniq_period_start' => $periodStart, 'uniq_period_end' => $periodEnd ]);
    }
    /** @return int|string|null */
    public function getIdBySubjectTableAndPeriodStartAndPeriodEnd(string $subjectTable, \DateTimeInterface|string $periodStart, \DateTimeInterface|string $periodEnd) {
        $row = $this->getBySubjectTableAndPeriodStartAndPeriodEnd($subjectTable, $periodStart, $periodEnd);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\MerkleRoots\Dto\MerkleRootDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\MerkleRoots\Dto\MerkleRootDto|null {
